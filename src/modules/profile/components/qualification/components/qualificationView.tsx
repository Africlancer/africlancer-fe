...
export const QualificationView: React.FC<Iprops> = ({ qualification, setModal, length }) => {
    ...
    {
        deleteQualification({ variables : {
            qualificationID: qualification._id
          }})
        .then((val) => {successMsg('Success', 'Qualification Has Been Deleted')})
    ...
    const items: MenuProps['items'] = [
        {
            key: '1',
            label: (
                <button onClick={() => {setModal({ open: true, data: qualification })}}>
                    Edit
                </button>
            ),
        },
            ...
            label: (
                ...
                >
                    <div className='mt-3'>
                        ...
                        <ApButton className='py-1 px-2 bg-skin-accent text-white rounded' onClick={deleteQualificationHandler}>Confirm</ApButton>
                    </div>
                </ApPopConfirm>
            ),
        },
  ...
  return (
    ...
    <div>
        ...
        <p className='font-bold'>Name of Conferring Organization - { qualification.conferringOrganization }</p>
